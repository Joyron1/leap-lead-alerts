...
# Usage: scripts/deploy.sh [lead-alert|daily-summary|leap-sync|lead-digest|keyword-report|push-send|all]
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ "$FN" = all ]; then for f in lead-alert daily-summary leap-sync lead-digest keyword-report push-send; do deploy "$f"; done; else deploy "$FN"; fi
