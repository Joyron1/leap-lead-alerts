...
| `dashboard/` | **https://leap-lead-alerts.vercel.app** — Vercel team `snap-loans-cash` (a different Vercel account from the one the Vercel MCP/CLI here are logged in to; don't create projects from here), Root Directory `dashboard`, auto-deploys every push to `main`. Locally: `npm run dev` (port 5180) / `start-dashboard.cmd` | React + Vite SPA, Hebrew/RTL. Reads Supabase directly with the **publishable** key; access is enforced by RLS: signed in + email in `public.dashboard_users` + email confirmed (`is_dashboard_user()`). Pages: overview (KPIs vs previous period, revenue chart, top sites/states), sites (health table), leads (filters, CSV). `src/lib/data.ts` is pure logic covered by `npm run selftest`; fetching is in `src/lib/api.ts`. See `dashboard/README.md`. |
| `public.dashboard_users` | Postgres (RLS, no policies) | Allow-list of dashboard emails. Seeded with jron81@gmail.com. |
