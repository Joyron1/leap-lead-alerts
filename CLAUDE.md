...
| `dashboard/` | Local for now (`npm run dev`, port 5180; Windows: `start-dashboard.cmd`), Vercel later | React + Vite SPA, Hebrew/RTL. Reads Supabase directly with the **publishable** key; access is enforced by RLS: signed in + email in `public.dashboard_users` + email confirmed (`is_dashboard_user()`). Pages: overview (KPIs vs previous period, revenue chart, top sites/states), sites (health table), leads (filters, CSV). `src/lib/data.ts` is pure logic covered by `npm run selftest`; fetching is in `src/lib/api.ts`. See `dashboard/README.md`. |
