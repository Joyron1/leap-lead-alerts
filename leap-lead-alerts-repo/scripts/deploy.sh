...
# Usage: scripts/deploy.sh [lead-alert|daily-summary|leap-sync|all]
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ "$FN" = all ]; then for f in lead-alert daily-summary leap-sync; do deploy "$f"; done; else deploy "$FN"; fi
