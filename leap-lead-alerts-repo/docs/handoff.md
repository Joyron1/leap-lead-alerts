...
## Status
- Everything is deployed and running: `lead-alert` (v11), `daily-summary` (v10), `leap-sync` (deployed manually by Joy via the dashboard editor), pg_cron jobs active.
...
## Pending change NOT yet deployed
`supabase/functions/leap-sync/index.ts` in this repo contains a newer version than production:
...
Deploy it with `scripts/deploy.sh leap-sync`.
